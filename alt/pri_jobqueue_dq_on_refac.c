...
/* 
 * copy_top_job: helper function to find the highest priority job on the queue
 * and copy to the dst pointer. Used by pri_jobqueue_dequeue and 
 * pri_jobqueue_peek. Note: the top_idx pointer is an out parameter to allow
 * this function to communicate the position of the highest priority job in 
 * the queue. This means that pri_jobqueue_dequeue does not have to search 
 * again for the position.
 */
static job_t* copy_top_job(pri_jobqueue_t* pjq, int* top_idx, job_t* dst) {
...
job_t* pri_jobqueue_dequeue(pri_jobqueue_t* pjq, job_t* dst) {
    int top_idx;
    dst = copy_top_job(pjq, &top_idx, dst);
...
job_t* pri_jobqueue_peek(pri_jobqueue_t* pjq, job_t* dst) {
    ...
    return copy_top_job(pjq, &top_idx, dst);
}
